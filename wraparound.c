...
int contain_vertical(char *string, char *word, int grid_row_len, int grid_col)
{
  while (1)
  {
    if (*string == '\0')
    {
      string = string - ((grid_col + 1) * (grid_row_len + 1));
    }
...
int contain_diagonal(char *string, char *word, int grid_row_len, int grid_col)
{
  while (1)
  {
    ...
    if (*string == '\0' || *string == '\n')
    {
      string = string - (grid_col * (grid_row_len + 2));
    }
...
void strfind()
{
  ...
  while (grid[grid_idx] != '\0')
  {
    ...
    for (idx = 0; idx < dict_num_words; idx++)
    {
      ...
      if (contain_vertical(grid + grid_idx, word, grid_row_len, grid_col))
      {
        word_found = 1;
      ...
      if (contain_diagonal(grid + grid_idx, word, grid_row_len, grid_col))
      {
        word_found = 1;
